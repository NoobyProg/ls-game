...
class GameCog(commands.Cog):
    ...
    def players_list(self, ctx):
        ...
        play_data = cur.execute("SELECT playerId FROM players WHERE gameId = ?",(g_id,)).fetchall()
        players = []
    ...
    async def start(self, ctx):
        if self.check_inGame(ctx) is False:
            ...
            if gamemaster_id != ctx.author.id:
                ...
                # Information Phase
                templates = ("Either `#` or `@` is L","`#` is also an Investigator", "Either `@` or `#` is L")
                ...
                for i in players:
                    ...
                    cur.execute("UPDATE players SET name = ? WHERE playerId = ?",(name, i,))
                ...
                # GAMEPLAY START #
                ##################
                player_data = cur.execute("SELECT * FROM players WHERE gameId = ?",(g_id,)).fetchall()

                g_channel = cur.execute("SELECT channelId FROM games WHERE guildId = ? AND gameId = ?",(ctx.guild.id, g_id,)).fetchone()[0]
                g_channel = await self.bot.fetch_channel(g_channel)

                await g_channel.send("**The L's Game Begins**\nThe Players are :-")
                for i in range(len(player_data)):
                    await g_channel.send(player_data[i][4])
